...
def CNN( X, y, datadir, classname ):
    ...
    y_pred_overall = np.zeros(shape=(0, 3))
    y_test_overall = []
    ...
    #for i in range(1):    # if simple hold-out split
        ...
        #y_pred_class = np.argmax(model.predict(X_test, batch_size=1, verbose=2))
        #y_pred_class = model.predict(X_test, batch_size=1, verbose=2)
        temp = model.predict(X_test, batch_size=1, verbose=2)
        y_pred_class = model.predict(X_test, batch_size=1, verbose=2).astype(np.float32)
        #print '\ny_train[0]', y_train[0], '\nY_train[0]', Y_train[0]
        #print 'y_pred_class[0]', y_pred_class[0]
        
        if k == 1:
            y_pred_overall = y_pred_class
...
if __name__ == '__main__':
    ...
    # load Tennessee Eastman Simulation data
